/*****************************************************************************
*                                                                            *
*  Vertex-invariants source file for nauty 2.0.                              *
*                                                                            *
*   Copyright (1989-2000) Brendan McKay.  All rights reserved.               *
...
#define MAXCLIQUE 8     /* max clique size for cliques() */
#define MAXINDSET 8     /* max independent set size for indsets() */

#if MAXN
static set workset[MAXM];             /* used for scratch work */
static shortish workshort[MAXN+2];    /* used for scratch work */
#else
...
UPROC 
cellfano2(g,lab,ptn,level,numcells,tvpos,invar,invararg,digraph,m,n)
graph *g;
nvector *lab,*ptn;
int level,numcells,tvpos,invararg,m,n;
permutation *invar;
boolean digraph;
{
        ...
        int icell,bigcells,cell1,cell2;
        shortish *cellstart,*cellsize;
	int nw,w01,w02,w03,w12,w13,w23;
	int pt0,pt1,pt2;
	set *gv0,*gv1,*gv2,*gv3;
	set *gp0,*gp1,*gp2;

#if !MAXN
	DYNALLSTAT(shortish,v,v_sz);
	DYNALLSTAT(shortish,w,w_sz);
        DYNALLOC1(shortish,workshort,workshort_sz,n+2,"cellfano2");
	DYNALLOC1(shortish,v,v_sz,n,"cellfano2");
	DYNALLOC1(shortish,w,w_sz,n,"cellfano2");
#else
	shortish v[MAXN],w[MAXN];
#endif
		...
		for (iv1 = iv0 + 1; iv1 <= cell2; ++iv1)
		{
		    v1 = lab[iv1];
                    ...
                    if ((w01 = uniqinter(gv0,GRAPHROW(g,v1,m),m)) < 0) continue;
		    v[nw] = v1;
		    w[nw] = w01;
		    ++nw;
		}	
			...
			if (ISELEMENT(gv1,v2)) continue;
                        gv2 = GRAPHROW(g,v2,m);
			if ((w12 = uniqinter(gv1,gv2,m)) < 0) continue;

                        for (iv3 = iv2 + 1; iv3 < nw; ++iv3)
                        {
			    w03 = w[iv3];
			    if (w03 == w01 || w03 == w02) continue;
                            v3 = v[iv3];
			    if (ISELEMENT(gv1,v3) || ISELEMENT(gv2,v3))
				continue;
                            gv3 = GRAPHROW(g,v3,m);
			    if ((w13 = uniqinter(gv1,gv3,m)) < 0) continue;
			    if ((w23 = uniqinter(gv2,gv3,m)) < 0
                                                   || w23 == w13) continue;

			    if ((pt0 = uniqinter(GRAPHROW(g,w01,m),
						 GRAPHROW(g,w23,m),m)) < 0)
				continue;
			    if ((pt1 = uniqinter(GRAPHROW(g,w02,m),
                                                 GRAPHROW(g,w13,m),m)) < 0)
                                continue;
                            if ((pt2 = uniqinter(GRAPHROW(g,w03,m),
                                                 GRAPHROW(g,w12,m),m)) < 0)
                                continue;

			    gp0 = GRAPHROW(g,pt0,m);
			    gp1 = GRAPHROW(g,pt1,m);
			    gp2 = GRAPHROW(g,pt2,m);
...
UPROC 
cellfano(g,lab,ptn,level,numcells,tvpos,invar,invararg,digraph,m,n)
graph *g;
nvector *lab,*ptn;
int level,numcells,tvpos,invararg,m,n;
permutation *invar;
boolean digraph;
{
	...
	DYNALLOC1(set,ww,ww_sz,m,"cellfano");
#else
	set w01[MAXM],w02[MAXM],w03[MAXM],w12[MAXM],w13[MAXM],w23[MAXM];
        set pt0[MAXM],pt1[MAXM],pt2[MAXM];
        set ww[MAXM];
#endif
        ...
        for (icell = 0; icell < bigcells; ++icell)
        {
			...
			for (i = M; --i >= 0;)
                            ww[i] = gv0[i] & gv2[i];
                        setnbhd(g,m,n,ww,w02);
                        for (i = M; --i >= 0;)
                            ww[i] = gv1[i] & gv2[i];
                        setnbhd(g,m,n,ww,w12);
				...
				continue;
                            ...
                            for (i = M; --i >= 0;)
                                ww[i] = gv0[i] & gv3[i];
                            setnbhd(g,m,n,ww,w03);
                            for (i = M; --i >= 0;)
                                ww[i] = gv1[i] & gv3[i];
                            setnbhd(g,m,n,ww,w13);
                            for (i = M; --i >= 0;)
                                ww[i] = gv2[i] & gv3[i];
                            setnbhd(g,m,n,ww,w23);
			
			    for (i = M; --i >= 0;)
				ww[i] = w01[i] & w23[i];
			    setnbhd(g,m,n,ww,pt0);
                            for (i = M; --i >= 0;)
                                ww[i] = w03[i] & w12[i];
                            setnbhd(g,m,n,ww,pt1);
                            for (i = M; --i >= 0;)
                                ww[i] = w02[i] & w13[i];
                            setnbhd(g,m,n,ww,pt2);
			    pc = 0;
			    for (i = M; --i >= 0;)
			    {
...
boolean digraph;
{
	...
	DYNALLOC1(set,sofar,sofar_sz,m,"distances");
	DYNALLOC1(set,frontier,frontier_sz,m,"distances"); 
#else
...
boolean digraph;
{
	...
	DYNALLOC2(set,s,s_sz,m,MAXINDSET-1,"indsets");
#else
	set s[MAXINDSET-1][MAXM];
#endif
        ...
        for (v[0] = 0; v[0] < n; ++v[0])
        {
            ...
            while (ss > 0)
            {
                ...
                else if ((v[ss] = nextelement((set*)s+M*(ss-1),M,v[ss])) < 0)
                    --ss;
                else
                {
...
boolean digraph;
{
	...
	DYNALLOC2(set,nset,nset_sz,m,MAXCLIQUE-1,"cliques");
#else
	set nset[MAXCLIQUE-1][MAXM];
	    ...
	    ns = (set*)nset;
            ...
            while (ss > 0)
            {
                ...
                else if ((v[ss] = nextelement((set*)nset+M*(ss-1),M,v[ss])) < 0)
                    --ss;
                else
                {
                    wv[ss] = wv[ss-1] + workshort[v[ss]];
...
boolean digraph;
{
	...
	DYNALLOC2(set,nset,nset_sz,m,MAXCLIQUE-1,"cellcliq");
#else
	set nset[MAXCLIQUE-1][MAXM];
#endif
		...
		ns = (set*)nset;
				...
				= nextelement((set*)nset+M*(ss-1),M,v[ss])) < 0)
                        --ss;
                    else
                    {
                        ++ss;
...
boolean digraph;
{
	...
	DYNALLOC2(set,nset,nset_sz,m,MAXCLIQUE-1,"cellind");
#else
	set nset[MAXCLIQUE-1][MAXM];
#endif
...
void
nautinv_check(wordsize,m,n,version)
